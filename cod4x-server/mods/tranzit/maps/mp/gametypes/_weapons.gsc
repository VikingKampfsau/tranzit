...
onPlayerSpawned()
{
	...
	for(;;)
	{
		...
		self thread scripts\weapons::watchWeapons();
...
watchWeaponChange()
{
	...
	if(self hasAttached("worldmodel_flametank"))
		self detach("worldmodel_flametank", "j_spine4");
	
	if(self.lastDroppableWeapon == getWeaponFromCustomName("riotshield"))
		self.leftWeaponWorldModel = "zmb_shield_worldmodel";
	else if(self.lastDroppableWeapon == getWeaponFromCustomName("knife"))
		self.leftWeaponWorldModel = "worldmodel_knife";
	else if(self.lastDroppableWeapon == getWeaponFromCustomName("flamethrower"))
		self.tag_stowed_back = "worldmodel_flametank";
	...
	while(1)
	{
		...
		if(isDefined(self.tag_stowed_back) && self hasAttached(self.tag_stowed_back))
		{
			if(self.tag_stowed_back == "worldmodel_flametank")
				self detach(self.tag_stowed_back, "j_spine4");
			else
		...
		{
			if(newWeapon == getWeaponFromCustomName("riotshield"))
				self.leftWeaponWorldModel = "zmb_shield_worldmodel";
			else if(newWeapon == getWeaponFromCustomName("knife"))
				self.leftWeaponWorldModel = "worldmodel_knife";
			else if(newWeapon == getWeaponFromCustomName("flamethrower"))
				self.tag_stowed_back = "worldmodel_flametank";
			else if(newWeapon == getWeaponFromCustomName("sentrygun"))
				self thread scripts\sentrygun::WaitForSentryDeploy(self.lastDroppableWeapon);
			else if(newWeapon == getWeaponFromCustomName("generator"))
				self thread scripts\generator::WaitForGeneratorDeploy(self.lastDroppableWeapon);
			...
			{
				if(self.tag_stowed_back == "worldmodel_flametank")
					self attach(self.tag_stowed_back, "j_spine4", false);
				else
					self attach(self.tag_stowed_back, "tag_stowed_back", false);
			}
		}
	}
}
...
watchGrenadeUsage()
{
	...
	self thread scripts\weapons::watchGrenadeUsage();
...
beginGrenadeTracking()
{
	...
	else if ( weaponName == getWeaponFromCustomName("monkeybomb") )
	{
		grenade thread scripts\monkeybomb::startMonkeyBomb();
		grenade.originalOwner = self;
	}
