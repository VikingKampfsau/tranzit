...
menuAllies()
{
	...
	{
		if(level.teamBased && !scripts\_include::TeamHasFreeSlots("allies"))
		{
			self setClientDvar("tranzit_max_survivors", "This is a gametype for 4 players. You can not join right now, please try again later.");
	...
	self setClientDvar("tranzit_max_survivors", "");	
...
menuAxis()
{
	...
	{
		if(level.teamBased && !scripts\_include::TeamHasFreeSlots("axis"))
		{
			self openMenu(game["menu_team"]);
			return;
		}
...
startGame()
{
	...
	thread scripts\spawnlogic::spawnPerFrameUpdate();
